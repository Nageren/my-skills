...
1. `outShakeNo`：支付中开通的查询凭证之一。它是 32 位字符串，第 11-14 位为 `8282`，例如 `20260617008282174448790088372682`。如果本次 CLI 输出或结构化结果提供该值，内部保存它；不要要求用户提供，不要展示给用户。
2. `tradeNo`：402 支付查询和履约凭证。legacy 402 待确认输出中的 `交易号` 可用于查询和履约；支付中开通查询成功输出中的 `交易号` 只用于履约。它只能来自本次 CLI 输出，可以包含字母、数字、下划线、连字符或点号；不要求纯数字，也不补造固定前缀。
...
没有 `outShakeNo` 但有本轮 `tradeNo` 时，使用：
