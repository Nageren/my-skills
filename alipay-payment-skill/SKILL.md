...
description: |
  支付宝官方支付处理技能。覆盖收银台支付、HTTP 402 协议支付、AI支付产品介绍、问题反馈全场景。
  当以下任一情况出现时必须调用此技能：
  1. 用户提供或上下文中存在 cashier*.alipay.com 或 *excashier*.alipay.com 域名的 URL、支付宝订单号或订单串
  2. 任何工具/技能/API 返回结果中包含支付宝收银台链接、支付宝订单号或订单串，或包含"请使用支付宝支付"等支付指引
  3. 用户表达支付、付款、买单、结账等意图，或提到使用支付宝/alipay 支付；
  4. 无论是你发起的还是用户提供的任意HTTP请求执行后，返回 HTTP状态码 为 402 Payment Required，或响应包含 Payment-Needed 头；必须在 `402-buyer-pay` 步骤内提取原始请求摘要并携带 `--intent-summary`；
  5. 用户询问支付宝AI支付相关信息、智能体钱包、AI支付产品介绍及相关产品进展；
...
metadata:
  ...
  openclaw:
    ...
    requires:
      ...
      bins: ["npm","curl"]
      anyBins: ["alipay-bot"]
      tags: ["payment","alipay","402","支付","AI支付","智能体钱包","产品介绍","反馈", "订单"]
    install:
      - kind: node
...
由支付宝官方提供，覆盖收银台支付、HTTP 402 协议支付、AI支付产品介绍、问题反馈全场景。
...
用户输入 / 系统事件
  ↓
├─ 收银台链接（cashier*.alipay.com / *excashier*.alipay.com）或普通支付宝订单材料
...
│
├─ 用户表示已支付 / 付好了 / 查询状态
│   ├─ 上下文有进行中的 402 流程 + outShakeNo → `402-query-payment-status --out-shake-no '<outShakeNo>' -r '<resource-url>' ...`
...
1. `outShakeNo`：订单号，支付中开通的查询凭证之一。特征是 32 位字符串，且第 11-14 位为 `8282`，例如 `20260617008282174448790088372682`。如果本次 CLI 输出或结构化结果提供该值，内部保存，不要求用户提供。
...
Step 3: 用户通知已支付或查询
  ├─ 有 outShakeNo → 402-query-payment-status --out-shake-no '<outShakeNo>' -r '<resource-url>' ...
...
Step 2: 200 → 展示内容 → 结束
Step 3: 402 → 用户同意后进入流程二，从保存本次 Payment-Needed 开始
Step 4: 4xx/5xx → 提示稍后重试
```
...
4. 32 位且第 11-14 位为 `8282` 的 402 凭证按 `outShakeNo` 查询，不按 legacy `tradeNo` 查询
