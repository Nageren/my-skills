...
**在 Hermes Agent 中，使用 `config bind` 而不是 `config init`。** `config init --force-init` 会进入 TUI 模式，在 background 中挂起无输出。

完整配置流程见：[`references/lark-cli-hermes-setup.md`](references/lark-cli-hermes-setup.md)（7 步：创建应用 → 写凭证 → bind → auth login → 二维码 → device-code → 开通 scope）
...
**URL 转发规则**：当命令输出 `verification_url`、`verification_uri_complete`、`console_url` 等 URL 字段时：**必须生成二维码**：你必须调用 `lark-cli auth qrcode` 将 URL 转为二维码并展示给用户，这是必须步骤，不要跳过。优先生成 PNG 二维码（--output）；**`--output` 必须是相对路径**（如 `./qr.png`），绝对路径（`/tmp/qr.png`）会被拒绝。仅当用户明确要求时才使用 ASCII（--ascii）。**URL 输出规则**：将 URL 视为不可修改的 opaque string，不要做任何修改（包括 URL 编码/解码、添加空格或标点、重新拼接 query），二维码和链接请一起展示给用户。
