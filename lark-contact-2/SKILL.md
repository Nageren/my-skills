...
- **Bot / 外部联系人搜不到**：`+search-user` 只返回企业通讯录中的人类用户。机器人、外部联系人、部分应用用户**不会出现在搜索结果中**。按姓名搜索返回空但确定对方存在时，可能是 bot 或外部联系人——改用 [`lark-im +chat-list --types p2p,group`](../lark-im/SKILL.md) 在聊天列表中查找，需要时翻页（`--page-token`）。
