...
要给某人发消息但不知道 `open_id` 或 `chat_id` 时：

1. **先尝试通讯录搜索**：`lark-cli contact +search-user --query "姓名" --as user`
2. **搜不到可能是 bot**：bot 不出现在通讯录搜索结果中。改用聊天列表找：
   ```bash
   lark-cli im +chat-list --types p2p,group --as user
   # 如果首屏没有，用返回的 page_token 翻页
   lark-cli im +chat-list --types p2p,group --as user --page-token "xxx"
   ```
3. **找到 chat_id 后发消息**：`lark-cli im +messages-send --chat-id oc_xxx --text "内容" --as user`

> **注意**：`+messages-search` 需要 `search:message` 和 `im:message.reactions:read` scope，当前用户身份通常未开通，不要依赖消息搜索来找人。
