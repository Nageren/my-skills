...
- ~~动画导出流水线：references/video-export.md~~ —— 已 scope-cut（delivery-artifact 集成不含视频导出，见 huashu-design.md 顶部通告）
- ~~音频配置（BGM + SFX 双轨）：references/audio-design-rules.md~~ —— 已 scope-cut（同上）
