...
> ⚠️ **delivery-artifact 集成裁剪**（本 vendor 加，上游原文不含）
>
> 本目录覆盖**到 HTML 产物为止**。配音 / TTS / BGM / SFX / 视频导出 / launch-film 工作流 **未纳入** —— 下游管线属另一项工作。下文 SOP 涉及这些子功能的步骤一律跳过。
>
> 详情、删除清单、重新同步命令，见同目录 `SOURCE.md`。
