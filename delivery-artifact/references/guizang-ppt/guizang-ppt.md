<!--
  来源:guizang-ppt-skill — https://github.com/op7418/guizang-ppt-skill(作者 歸藏 / op7418,MIT)
  这是上游 SKILL.md 的引用版,作为 delivery-artifact 的「PPT 风」形态使用。
  与上游的差异 + 重新同步方式见同目录 SOURCE.md;许可见同目录 LICENSE。
  注:本文件不带 skill frontmatter、且不命名为 SKILL.md —— 它是 reference,不是独立 skill。
-->
...
| 5 | **有没有图片或截图?希望怎么处理?** | 决定图文版式、图片槽位、截图是否需要 GPT-M 2.0 重构 |
...
默认策略:先让内容适配模板,再处理图片比例。截图需要保真展示时,按标准比例直接落位、尽量不重画截图内容;只有原截图太乱、太长、太窄或需要概念化表达时,才用 GPT-M 2.0 做截图再设计。
...
如果用户提供的是截图,先判断是**截图保真**还是**截图再设计**:

- 截图保真:按标准比例直接落位,尽量不重画截图内容
...
- 如果处理用户原始截图,需要重构信息时才用 GPT-M 2.0 重画(`references/image-prompts.md`),否则按标准比例保真落位
...
- 正文页只能使用 `references/swiss-layout-lock.md` 登记的 22 个版式 `S01-S22`;新增首页/尾页只能使用 Skill 明确提供的 `SWISS-COVER-ASCII` / `SWISS-CLOSING-ASCII`。
...
1. 跑生成的 `index.html`,**同时打开 `assets/template-swiss.html`** 看基础渲染基线(基础设施 + 封面/封底两个示例,展示主题预期效果)。
2. 截图前等入场动效稳定(约 1-2 秒),不要把动画中间态当成版式问题。
3. 视觉判断按 `references/layouts-swiss.md` 末尾的「视觉锚点表」逐项对照(大标题字重、留白、分割线、Timeline、图片页规则)—— 这是从 op7418 原 PPT 里蒸馏出的判断锚点,等价于直接看原 PPT。
4. 代码判断:确认该页选用的版式与内容形状匹配,没有把数据专用版式拿来讲概念,也没有把可选组件堆成装饰。
5. 如果页面别扭,先判断是版式选错、必选组件缺失、可选组件滥用,还是间距/安全区问题;不要直接靠加 margin 硬救。
...
```
guizang-ppt/
├── guizang-ppt.md            ← 你正在读(本文件)
├── SOURCE.md                 ← 上游来源、与上游的差异、重新同步方式
├── LICENSE                   ← 上游 MIT 许可（歸藏 / op7418）
├── assets/
...
6. 如果在 Codex 中生成配图,读 `image-prompts.md` 挑图片类型、比例和基础提示词;用户原始截图按标准比例保真落位,需要重构信息时才用 GPT-M 2.0 重画
