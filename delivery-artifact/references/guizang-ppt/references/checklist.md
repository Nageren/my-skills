...
**根因**:生成时把 Swiss 当成风格包,自由组合了新的 P23/P24/自绘 SVG 页面,没有从 `swiss-layout-lock.md` 登记的 22 个版式里选。
...
**现象**:最顶上的中文标题在页面中间,像一页自制海报,不再像 swiss 基线。
...
- 用户原始截图按标准比例保真落位、用 `.fit-contain` 保留全部文字,不要为了比例统一就重画截图内容
...
**现象**:生成页看起来像瑞士风,但实际字重、间距、时间线、卡片密度不一致;越迭代越偏离基线。

**根因**:把新增图片版式或实验结构写成了全局样式修改,或无意改动了原始基座类,例如 `.h-hero` / `.h-xl` 字重、`.tl-node` 列宽、`.duo-compare` 间距。

**做法**:
- 视觉基线 = `assets/template-swiss.html`(基础设施 + 封面/封底示例)+ `references/layouts-swiss.md` 末尾的「视觉锚点表」(从 op7418 原 PPT 蒸馏出的判断锚点,等价于看原 PPT)
...
- 如果要修改 `assets/template-swiss.html`,先与基线对比;可接受差异只应是 ASCII 类、S22 图片定位类、轻字重标题 helper 和已知动效修复

**自检命令**:
- 运行本次测试目录里的 `compare-swiss-base.mjs`,确认输出里 `missing in template: 0`
- 跑生成页打开浏览器,对照「视觉锚点表」逐项判断:大标题字重、chrome-min 位置、timeline dot/label、卡片密度
...
**做法**:
- 跑生成页 + 同时打开 `assets/template-swiss.html` 看基础渲染基线,对照 `references/layouts-swiss.md` 末尾「视觉锚点表」做并排判断
- 等入场动效稳定后再截图或下判断,不要把动画中间态当成内容缺失
- 先打开网页逐页看视觉:标题字重、头部间距、正文密度、图片对齐、nav 安全区
- 再回代码看结构:该页是否用了正确版式,必选组件是否齐,可选组件是否过度
- 对照 `template-swiss.html` 与「视觉锚点表」时以实际画面为准;raw CSS helper 只能辅助,不能替代视觉判断
