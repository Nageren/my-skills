...
视觉基线 = `assets/template-swiss.html`(基础设施 + 封面/封底示例)+ 本文件末尾的「视觉锚点表」。详见 `references/swiss-layout-lock.md` 的「视觉基线」节。
...
- 大标题遵循 swiss 基线的实际页面用法:主标题 `font-weight:200`,重点词/数字 `font-weight:300`;不要因为旧 CSS helper 里残留过 800/900 就把 Swiss 大标题加粗
...
1. 跑生成的页面 + **同时打开 `assets/template-swiss.html`** 看基础渲染基线(封面 + 封底两个填好的示例,展示主题预期效果)。
2. 截图前先等入场动效稳定(约 1-2 秒)。不要把动画中间态误判成"内容缺失"或"版式空白"。
3. 视觉判断按下方「视觉锚点表」逐项对照:标题字重、头部距离、图片落位、底部安全区、caption 是否被 nav 挡住。
4. 再回到代码,检查该页是否误用了不属于该版式的组件,例如把 P24 的三图证据墙塞进 P23,或把 P7 图表用于没有真实数值的概念列表。
5. 若视觉不一致,优先判断是**版式选择错**、**必选组件缺失**、**可选组件滥用**还是**间距/安全区问题**,不要直接靠调 `margin` 硬救。
6. 修改模板时,新增能力必须用新类隔离;不要因为一页出问题去改全局基座类。
...
> 这份锚点表是从 op7418 原 ground-truth PPT 里蒸馏出的判断点。原 PPT 在他私人 dev 项目里未开源;有这张表 + `template-swiss.html` 已经够用,**不需要再去找那份 PPT**。
...
- 如果用户原始截图比例混乱,先按标准比例保真落位、用 `.fit-contain` 保留原图;只有太长、太窄或需要重构信息时,才用 GPT-M 2.0 重生成同一比例的"截图再设计"
