...
本主题的视觉 ground truth 由两份文件共同定义:

- `assets/template-swiss.html` —— 基础设施 + 封面/封底两个示例(展示主题预期渲染)
- `references/layouts-swiss.md` —— 22 个登记版式的可粘贴 HTML 块 + 末尾「视觉锚点表」

op7418 dev 时引用过他本机的成品 PPT 当 ground truth,该文件未开源;上面两份是它的等价替代 —— 锚点表是从那份 PPT 里蒸馏出的判断点,模板是基础渲染基线。
...
- 如果原始截图比例不可控,先按标准比例保真落位、用 `.fit-contain` 保留原图;只有长截图、极窄截图或信息需要重构时,才用 GPT-M 2.0 重生成“截图再设计”。
